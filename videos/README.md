...
rolling-релиз свежими файлами.

**Имя файла — это КУРС, а не внутренний id.** `id` продакшена («lebed») —
короткое рабочее имя для маркера `[only: …]`, гейтов и голденов; в релизе оно
не говорит студии ничего. Файлы называются полем `kurs` из манифеста — тем же
слагом, что стоит в адресе курса на сайте:

| файл | что это |
|---|---|
| `chernyj-lebed.mp4` | **готовый ролик со звуком** — то, что смотрят |
| `chernyj-lebed-nemoj.mp4` | та же картинка без звука |
| `chernyj-lebed-voice.mp3` | голосовая дорожка отдельно |

Немая версия названа длиннее готовой намеренно: раньше было наоборот
(`lebed.mp4` — немая, `lebed-final.mp4` — готовая), и скачивали немую.
