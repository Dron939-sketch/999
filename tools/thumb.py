...
def to_169(im):
    ...
    if w / h < 16 / 9:                    # кадр слишком высокий — режем по высоте
        th = int(w * 9 / 16)
        if y1 - y0 <= th:                 # сюжет влезает — ставим окно по нему
            top = (y0 + y1) // 2 - th // 2
        else:
            # НЕ ВЛЕЗАЕТ — ЖЕРТВУЕМ НИЗОМ, НЕ ВЕРХОМ. Центровка по сюжету здесь
            # срезает персонажу макушку, и обложка с обрубленной головой мертва,
            # а ноги стульев за нижней кромкой не замечает никто: предмет,
            # уходящий за край, читается как кадр, а не как брак.
            top = y0 - int(th * 0.04)
        return im.crop((0, max(0, min(top, h - th)), w,
                        max(0, min(top, h - th)) + th))
...
def scrim(im, rect, target=BG_MAX - 20):
    ...
    # Нижнего порога у скрима нет. Пластина бывает уже чёрной — тогда затемнять
    # нечего, а «на всякий случай» скрим гасит то немногое, что в тени осталось:
    # у первой обложки так пропадала подсветка ближнего стула.
