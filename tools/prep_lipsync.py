...
#
# ЗАКРЫТЫХ БУКВ У RHUBARB РОВНО ДВЕ — A (М/Б/П) и X (покой). Все остальные
# рот ОТКРЫВАЮТ, и B в том числе: по документации это «слегка приоткрытый рот»,
# а не сомкнутый. Держит эту разницу гейт `tools/rot.py`.
#
# G — это Ф/В, губа на зубах: ближе к трубочке visF, чем к приоткрытому visB
# (в риге `mouth_f` так и подписан — «У/В»).
RHUBARB_MAP = {
    "A": "visA", "B": "visB", "C": "visC", "D": "visD",
    "E": "visE", "F": "visF", "G": "visF", "H": "visC", "X": "visA",
}

# Буквы Rhubarb, при которых рот ЗАКРЫТ. Всё остальное обязано его открывать.
ZAKRYTYE_BUKVY = {"A", "X"}
...
def gate_viseme(pose, prev_pose, dur, loud):
    ...
    ПОРОГ ПОНИЖЕНИЯ — 0.26, А НЕ 0.40, И ЭТО ЗАМЕР, А НЕ ВКУС. `loud` меряется
    от ПИКА СВОЕЙ ЖЕ реплики, а у речи медиана огибающей внутри реплики около
    0.34: порог 0.40 понижал 62% кадров, то есть «тихим» объявлялся обычный
    разговорный слог, а не шёпот. Диапазон рта схлопывался в одну форму.
    0.26 оставляет понижение примерно трети кадров — настоящему тихому хвосту
    фразы."""
