...
SCAFFOLD = re.compile(
    r"^\s*(together\s*\{|do\s*\{|\{|\}|//.*)?\s*$"
    # Пропы, выставленные ПЕРЕД речью: `imya moves-to ...`, `board rotates ...`.
    # Их писали между маркером и `speaks`, и маркер об них обнулялся: во второй
    # части «Худшего собеседника» так потерялся `//lip 1`, карта блоков вышла
    # [2..8], и вся дорожка легла на реплику раньше — до самого «Пип».
    r"|^\s*[A-Za-z_]\w*\s+(moves-to|scales|rotates|fades|opacity|layer)\b.*$")
...
def main(argv):
    ...
    # СВЕРКА ВТОРАЯ: каждый ли `//lip` нашёл себе `speaks`. Раньше потерянный
    # маркер был виден только в строке «карта блоков», и её никто не читал —
    # а стоит она дорого: голос собирается ПО КАРТЕ, поэтому один выпавший
    # блок сдвигает всю дорожку на реплику и рассинхронивает ролик целиком.
    markers = [int(m.group(1)) for m in re.finditer(r"^\s*//lip\s+(\d+)\s*$",
                                                    text, re.M)]
    lost_markers = [n for n in markers if n not in order]
    if lost_markers:
        sys.exit(f"  ✗ маркеры {', '.join('//lip ' + str(n) for n in lost_markers)} "
                 f"не нашли `speaks for` — между маркером и речью стоит строка, "
                 f"которую липсинк не считает строительной. Карта блоков вышла "
                 f"{order}: голос собирается по ней, и весь ролик уедет на "
                 f"реплику. Перенеси такие строки ВЫШЕ маркера.")
