...
ГДЕ ЭТА ПОГРЕШНОСТЬ КУСАЕТСЯ (замер 08.09.2026, «Философия»). Локации ролика
свели в один язык: вместо трёх тональных полос — светлое поле бумаги и одна
линия земли. СЦЕНАРИЙ НЕ МЕНЯЛСЯ НИ НА СТРОКУ, и каждая склейка локаций как
меняла крупность, так и меняет: все шесть сцен входят на общем плане, а
предыдущая выходит на среднем, крупном или сверхкрупе. Замер при этом:

    сильных смен кадра   11 → 7      (детектор ffmpeg, порог 0.55)
    планов               39 → 43
    доля вспышек <0.25с  0.46 → 0.51
    scale_jump           0.47 → 0.43   при пороге приёмщика 0.45

Обе причины — про пиксели, а не про монтаж. Склейка со светлого поля на
светлое двигает мало блоков и в сильные смены не попадает; зато движение
фигуры на пустом поле даёт всплеск диффа и режет план надвое. Числитель
теряет настоящие каты, знаменатель набирает ложные, и мягкий приёмщик краснеет
на ролике, у которого монтаж не изменился.

Возврат крупной тёмной массы в кадр (раструб рупора вдвое больше, контактные
тени под силуэтами) поднял замер до 0.44 и сильные смены до 9 — то есть
масса в кадре лечит и картинку, и прибор. Догонять последнюю сотую правкой
картины НЕ НАДО: это будет подгонка под детектор, а не режиссура
(`RULES.md`, «зелёный гейт не означает похожести»).
