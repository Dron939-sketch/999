...
ВТОРАЯ ПРОВЕРКА: ПРОП НА ТЕЛЕ И ДВИЖЕНИЕ КАМЕРЫ.
Проп живёт в координатах КАДРА, а не на фигуре. Пока камера стоит, разницы не
видно; стоит камере поехать (`camera medium/close`), персонаж приближается, а
метка остаётся где была — и «съезжает с балахона на картинку». Так студия
увидела номер курса.

Ловится машинно: если между `shows`/объявлением пропа и его `hides` встречается
смена плана, это дефект. Правило простое — проп на теле живёт только на
неподвижной камере.
...
def prop_vs_camera(text):
    """Проп на теле, переживший смену плана: метка съедет с фигуры.

    Ищем в каждой сцене отрезки между показом пропа и его снятием и смотрим,
    нет ли внутри `camera <план>`. Пропы, которые СПЕЦИАЛЬНО стоят в мире
    (тень, дверь, столб), в этот список не попадают: правило касается только
    тех, что объявлены как метка на фигуре — в имени `nomer`.
    """
    import re as _re
    bad = []
    for scene in _re.split(r"\nscene\s", text):
        lines = scene.splitlines()
        live = {}
        for i, ln in enumerate(lines):
            m = _re.search(r'let\s+(\w*nomer\w*)\s*=\s*prop', ln)
            if m:
                live[m.group(1)] = i
            m = _re.search(r"(\w*nomer\w*)\s+hides", ln)
            if m and m.group(1) in live:
                live.pop(m.group(1))
            if _re.search(r"camera\s+(medium|close|extreme|angle)", ln):
                for name in list(live):
                    bad.append((name, ln.strip()[:60]))
                    live.pop(name)
    return bad
