...
ГЛАВНАЯ ЛОВУШКА: ДО ОЗВУЧКИ ДЫРЫ НЕ ВИДНО. В сценарии содержимое сцены совпадает
с объявленной длительностью, и таймлайн выглядит чистым. Но липсинк подменяет
`speaks for` РЕАЛЬНОЙ длиной mp3, а синтез короче расчёта примерно на четверть:
содержимое сжимается, объявленное число остаётся полом — и разница уезжает в
готовый ролик тишиной. Так в «Привязанности» набежали три дыры по семь-восемь
секунд, каждая ровно в конце сцены, и ни одну из них гейт по сценарию не увидел.

Отсюда правило, которое проверяется статически: У ГОВОРЯЩЕЙ СЦЕНЫ `duration:` —
ПОЛ, А НЕ ЦЕЛЬ. Ставится 1s, длину задаёт содержимое, и тогда сцена сжимается
вместе с озвучкой. Держать кадр дольше нужно `wait`-ом: он виден в тексте и
его видит гейт. Сцена без реплик (титры) — исключение: там объявленная
длительность и есть содержимое.
...
FLOOR_MAX = 2.0     # выше этого `duration:` говорящей сцены — уже не пол, а цель


def scenes(path):
    """(имя, объявленная длительность, есть ли реплики) по каждой сцене."""
    s = Path(path).read_text(encoding="utf-8")
    m = list(SCENE.finditer(s))
    out = []
    for i, sc in enumerate(m):
        body = s[sc.end():m[i + 1].start() if i + 1 < len(m) else len(s)]
        out.append((sc.group(1), float(sc.group(2)), bool(LIP.search(body))))
    return out


def padding(path):
    """Говорящие сцены, объявившие длительность выше пола.

    Статическая проверка того, что по таймлайну сценария не поймать: объявленное
    число сработает подкладкой только ПОСЛЕ озвучки, когда содержимое сожмётся.
    """
    return [(n, d) for n, d, talks in scenes(path) if talks and d > FLOOR_MAX]
...
def check(path):
    ...
    for name, dur in padding(path):
        bad.append((f"сцена «{name}»", dur,
                    f"объявлена длительность {dur:g}s при пороге {FLOOR_MAX:g}s — "
                    f"после озвучки содержимое сожмётся, и разница уедет тишиной; "
                    f"ставь 1s, держи кадр `wait`-ом"))
...
def main(argv):
    ...
    for f in files:
        ...
        for idx, gap, why in bad:
            where = idx if isinstance(idx, str) else f"после реплики {idx}"
            print(f"      {where}: {gap:.1f} с — {why}")
