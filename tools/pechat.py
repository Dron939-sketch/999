...
def postavit_vychitku(path):
    ...
    # Пробел после печати ставится здесь: сама печать съедает тот, что стоял
    # в тексте, и абзац каждый раз слипался.
    t = t[:m.start()] + f"**ВЫЧИТКА (реплики {h}):** " + t[m.end():].lstrip(" ")
