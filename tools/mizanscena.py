...
#  МАСШТАБ ЧИТАЕТСЯ ВМЕСТЕ С ХВОСТОМ `over ...`, И ЭТО НЕ МЕЛОЧЬ.
#
#  Прежнее выражение требовало конца строки сразу за числом — `scales 0.86$`.
#  А закон завода прямо запрещает голый `scales`: он стоит полсекунды
#  таймлайна, поэтому во ВСЕХ роликах серии написано `scales 0.86 over 0.01s`.
#  Значит выражение не совпало ни разу ни в одном ролике, и и лист мизансцен,
#  и гейт масштаба тихо подставляли `1.0`.
#
#  Цена молчания. Контактный лист рисовал фигуру не того размера, что уедет в
#  ролик, — то есть «посмотреть глазами» проверяло не тот кадр. Гейт масштаба
#  (`tools/masshtab.py`) рендерил пробу в масштабе 1.0 и сверял с `chelovek`
#  ЕЁ, а не то, что стоит в раскадровке: число не менялось от `scales` вовсе.
#  Поймано на «Стоицизме» — гейт выдавал 0.47 и при 0.63, и при 0.40.
#  Подпись на самом листе всё это время честно печатала «1.0» в каждой сцене.
SCALES = re.compile(r"^\s*(\w+)\s+scales\s+([\d.]+)\b.*$", re.M)
