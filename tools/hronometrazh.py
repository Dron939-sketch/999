...
# Порог, ниже которого править нечего. ПОСТАВЛЕН ПО ЗАМЕРУ, А НЕ ИЗ ГОЛОВЫ.
#
# «Эмоции» озвучены дважды подряд ОДНИМ И ТЕМ ЖЕ текстом, с теми же ремарками и
# тем же темпом. Реплики разошлись между прогонами: медиана 0.40 с, максимум
# 1.49 с (VO-8: 8.78 → 7.29). Синтез не детерминирован, и это не сбой — так
# работает движок.
#
# Рядом — ошибка мерки, ради которой всё затевалось: медиана 0.77 с, максимум
# 1.98 с. То есть систематическая ошибка вдвое больше дрожания, но одного
# порядка с ним.
#
# Отсюда порог. Ниже 0.45 с править бессмысленно: там уже не ошибка расчёта, а
# шум синтеза, и «уточнение» переписывало бы сценарий на КАЖДОМ прогоне,
# гоняясь за числом, которого в следующий раз не будет. Выше — настоящая
# систематическая ошибка, её и снимаем.
#
# ЧТО ИЗ ЭТОГО СЛЕДУЕТ ПРО САМ `speaks for`. Он никогда не будет точен до
# кадра, потому что голос пересинтезируется на каждом прогоне. Он и не должен:
# точный синхрон держит `prep_lipsync`, который подменяет речь дорожкой рта ИЗ
# ЭТОГО ПРОГОНА. Здесь снимается смещение, чтобы гейты и каты считались от
# правды, а не от 0.36 с на слово.
TOLERANCE = 0.45
