...
def step_mux(prod, video_mp4, voice_mp3, sfx_mp3, out_final):
    ...
    if not have_voice and not have_sfx:
        ...
        return None
    # РОЛИК С МОНОЛОГОМ БЕЗ ГОЛОСА НЕ СОБИРАЕТСЯ ВОВСЕ.
    #
    # Здесь стоял запасной путь «нет голоса — возьмём дорожку эффектов», и он
    # один раз сработал ровно так, как не надо. Fish отдал HTTP 500, голоса не
    # стало, в сведение ушёл комнатный тон на −30.1 LUFS, а мастеринг — он ведь
    # настроен на РЕЧЬ — поднял его до −15.1 и выдал файл с именем готового
    # ролика. Студия получила две минуты ровного гула.
    #
    # Дорожка эффектов сама по себе законна: ролик без `vo` — это ролик без
    # монолога, и подложка в нём и есть весь звук. Незаконно другое: выдавать
    # подложку за ролик, который заявлен говорящим. Немой рендер честнее —
    # он и называется `-nemoj`, и в примечании к релизу объяснён.
    if not have_voice and prod.get("vo"):
        log("  [сведение] ролик заявлен с озвучкой (`vo`), а голоса нет — "
            "финального файла не будет. Немой рендер честнее, чем подложка, "
            "поднятая мастерингом до громкости речи.")
        return None
    if not have_ffmpeg():
    ...
    if have_voice and have_sfx:
        ...
        # КЛЮЧ БОКОВОЙ ЦЕПИ ДОТЯГИВАЕТСЯ ТИШИНОЙ, ИНАЧЕ ОН ОБРЕЗАЕТ ПОДЛОЖКУ —
        # А ВМЕСТЕ С НЕЙ КОНЕЦ РОЛИКА.
        #
        # `sidechaincompress` кончается вместе с КОРОЧЕ из двух входов, и
        # короче оказывался ключ: голос кончается на последней реплике, а
        # подложка живёт до конца картинки. Прижатая дорожка обрезалась по
        # голосу, `amix=duration=longest` складывал два одинаково коротких
        # входа, и сведённый звук выходил ровно длиной голоса. Дальше
        # `compose_video.sh` ставит `-t` по длине ЗВУКА — и обрезал картинку.
        #
        # На ролике 25 это стоило КОЛЬЦА. Последние 1.82с картинки — кадр,
        # тождественный первому, тот самый возврат якоря, на котором держится
        # зацикленность; звука там нет по замыслу («обрыв — глухая тишина до
        # конца»). Ровно поэтому его и срезало: 160.62с картинки против 158.41с
        # голоса, выдано 158.42с, от кольца не осталось ничего.
        #
        # `apad` на ключе бесконечен, но цепь всё равно кончается вместе с
        # подложкой — она второй вход. Тишина в ключе означает «не прижимать»,
        # так что хвост подложки идёт в полный уровень, как и задумано.
...
def qc_production(prod, video_mp4, final_mp4, voice_expected, voice_produced):
    ...
    HARD/SOFT — ЭТО ПРО ГОДНОСТЬ РОЛИКА, А НЕ ПРО ВИНОВАТОГО. Раньше делили по
    вине: наш баг — hard, внешний сбой (TTS лёг) — soft, «файлы всё равно
    отдаём». На этом ролик без голоса уехал в релиз при ЗЕЛЁНОМ прогоне, и
    поймала его студия, а не завод. Виноват действительно был Fish, но студии
    от этого не легче: заявленный говорящим ролик без монолога не отдаётся
    никуда, чей бы сбой его ни сделал. Отдать нечего — значит красный прогон.

    «Отдать хоть что-то» при этом не потеряно: заливка в релиз стоит под
    `if: always()`, поэтому немой рендер уезжает и при красном. Красным
    помечается не файл, а ПРОГОН — иначе в релизе молча остаётся mp4 прошлой
    сборки, и скачавший его думает, что держит в руках пересборку.

    `voice_produced` разделяет две причины: голос не собрался вовсе (внешний
    TTS) или собрался и потерялся в сведении (наш баг микса). Обе — hard,
    но чинятся по-разному, поэтому и сообщения разные."""
    hard, soft = [], []
    ...
    if voice_expected and prod.get("vo"):
        ...
        # ПЕРВЫМ ДЕЛОМ — БЫЛ ЛИ ГОЛОС, а не «есть ли в финале дорожка».
        # Проверка `has_audio` здесь стояла одна и пропустила ровно тот
        # случай, ради которого писалась: дорожка в финале БЫЛА — из одних
        # эффектов. Наличие дорожки не значит наличия монолога; за монолог
        # в готовом файле отвечает `tools/prosmotr.py` (доля речевой полосы),
        # а здесь мы ловим причину раньше и дешевле — по факту синтеза.
        if not voice_produced:
            hard.append(f"{prod['id']}: заявлен vo, но голос не сгенерился "
                        "(вероятно недоступен TTS) — готового ролика нет, "
                        "уехал только немой рендер")
        elif not Path(final_mp4).exists() or not has_audio(final_mp4):
