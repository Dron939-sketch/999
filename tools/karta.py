...
def render(rig_dir, pose, y, s):
    ...
    # ИМЯ СТЕНДА УНИКАЛЬНО НА ПРОЦЕСС. Файл пишется, рендерится и удаляется в
    # `finally` — пока стендом пользовался один гейт, общего имени хватало. Их
    # стало трое (осанка, разворот, силуэты), и два прогона рядом начали
    # затирать файл друг у друга: движок падал на полупустом сценарии, а гейт
    # честно сообщал «поза не отрисовалась» — то есть обвинял позу в чужой
    # гонке. Отлаживать такое дорого: падение плавающее и зависит от того, что
    # ещё запущено.
    #
    # Стенд обязан лежать РЯДОМ с ригом (относительные import'ы в .anim
    # разрешаются от папки сценария), поэтому уносить его в /tmp нельзя —
    # только развести по имени.
    src = ROOT / "examples" / "lektorij" / f"_karta_stand_{os.getpid()}.anim"
