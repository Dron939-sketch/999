...
def parse_vo_table(md_path):
    ...
    with open(md_path, encoding="utf-8") as f:
        for line in f:
            ...
            cell = quoted[-1] if quoted else (cells[-1] if cells else m.group(3))
...
def direct_line(mp3_bytes, remark, base=1.0):
    ...
    # Общий потолок дикции. 1.45, а не 1.35: на разогнанном ролике (база 1.30)
    # низкий потолок съедал сам приём — и пулемёт, и рассуждение упирались в
    # него и звучали одинаково быстро. Разница между битами важнее последних
    # процентов внятности, а 1.45 синтез ещё держит.
    tempo = min(1.45, max(0.75, tempo))
