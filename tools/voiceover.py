...
def direct_line(mp3_bytes, remark):
    ...
    if any(k in r for k in ("презрен", "ухмыл", "фамильярн", "дерзк")):
        tempo *= 0.96
    # --- ИРОНИЯ: легче и чуть быстрее, верх открыт --------------------------
    # Ирония — не презрение: она играется НАВЕРХУ, лёгким нажимом и быстрым
    # шагом, будто говорящему смешно раньше слушателя. Плотный низ её убивает.
    if any(k in r for k in ("ирони", "насмешл", "язвит", "подкол", "с усмешкой")):
        af += ["highpass=f=90", "volume=0.97"]
        tempo *= 1.04
    # --- СНИСХОДИТЕЛЬНО: тяжелее и медленнее, сверху вниз -------------------
    # Ответ на иронию по закону смены полярности (SCRIPT-FREEMAN §2a): там,
    # где ирония лёгкая и быстрая, снисходительность тяжёлая и медленная —
    # иначе две соседние реплики звучат одинаково и качель не читается.
    if any(k in r for k in ("снисходит", "свысока", "покровительств", "с ленцой")):
        af += ["lowpass=f=9000", "volume=1.04"]
        tempo *= 0.93
