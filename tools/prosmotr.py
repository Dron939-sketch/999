...
def zakadrovyj(video):
    """Снят ли готовый файл БЕЗ персонажа в кадре — по его же сценарию.

    Имя файла — слаг курса (`kurs` из манифеста), по нему находится продакшен,
    по нему — раскадровка. Закадровым считается ролик, где рассказчик хотя бы
    раз скрыт (`hides`) и ни разу не показан (`shows`).
    """
    import json
    imya = Path(video).stem
    for hvost in ("-nemoj", "-final", "-vert"):
        if imya.endswith(hvost):
            imya = imya[:-len(hvost)]
    try:
        ...
        return False
    for prod in d["productions"]:
        if (prod.get("kurs") or prod["id"]) != imya:
            continue
        anim = ROOT / prod.get("anim", "")
        if not anim.exists():
            return False
        t = anim.read_text(encoding="utf-8")
        est_hides = re.search(r"^\s*\w+\s+hides\b", t, re.M)
        est_shows = re.search(r"^\s*\w+\s+shows\b", t, re.M)
        return bool(est_hides) and not est_shows
    return False
...
def main(argv):
    ...
    #  ПРИЗНАК БЕРЁТСЯ ИЗ СЦЕНАРИЯ, А НЕ ИЗ КАРТИНКИ, и это выяснилось дорого.
    #
    #  Первая попытка считала ролик безфигурным, если маска не находится ни в
    #  одном кадре. На «Парусе» это не сработало: там ЕСТЬ человек — гребец,
    #  нарисованный прямо в локацию, и приёмщик честно находит его голову
    #  (светлое пятно, две тёмные точки глаз, черта рта). После чего меряет
    #  его мерками ГОВОРЯЩЕГО персонажа: рост ниже 35% кадра, фон темнее
    #  силуэта. Обе мерки написаны про мимику и осанку рига — у рисунка нет ни
    #  того, ни другого, читать в нём нечего, и претензия повисает в воздухе.
    #
    #  Отличает одно от другого только сценарий: закадровый ролик тот, где
    #  рассказчик объявлен и СКРЫТ (`hides`) и ни разу не показан (`shows`).
    #  Подделать это нельзя — ролик, где персонаж просто забыт, `hides` не
    #  содержит вовсе, и мерки к нему применятся по-старому.
    bez_figury = zakadrovyj(a.video)
    if bez_figury:
        print("  · ролик закадровый: рассказчик объявлен и скрыт, персонажа "
              "в кадре нет — мерки по фигуре не применяются")
        melko, lico, tmy = [], [], []
    if not bez_figury:
