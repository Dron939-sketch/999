...
def _close_x(dark, k):
    """Горизонтальное замыкание шириной k: заращивает просветы у́же k.

    ПОЧЕМУ ЭТО НЕ КОСМЕТИКА. Плащ мерится как самый длинный СПЛОШНОЙ тёмный
    отрезок в строке, и любая светлая царапина поперёк корпуса делит его
    надвое. У профиля ровно такая царапина есть: рука с белой каймой и трость
    лежат НА плаще и режут строку просветами в 1–3 пикселя. Гейт послушно
    отдавал ширину куска — 118 вместо 179, — и профиль годами проходил
    проверку «плащ 0.60–0.80 от анфаса» с фактическими 0.95. Хуже: чтобы
    поднять липовые 0.52 до липовых 0.63, плащ в позах разворота был РАСШИРЕН
    на 26% — правился рисунок под сломанную мерку.

    Порог 4 разделяет два вида просветов, и они отличаются на порядок:
    белая кайма руки — 1–3 пикселя, воздух между рукой и корпусом в анфасе —
    35–70. Замыкание сшивает первое и не трогает второе.
    """
    a = dark
    di = a.copy()
    for d in range(1, k + 1):
...
def measure(frame):
    ...
    # Сперва СШИТЬ царапины (белая кайма руки, трость), потом СТЕРЕТЬ тонкое
    # (сама рука на просвете). Порядок важен: открытие по нецелому плащу
    # оставляет обрубок, и мерка врёт — см. _close_x.
    body = _open_x(_close_x(dark, 4), 12)
