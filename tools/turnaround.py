...
def _fill_holes(dark):
    """Тёмное с залитыми внутренними полостями — то есть силуэт.

    Своё, а не scipy: у приёмщика из внешнего только numpy, а `label` берётся
    из karta. Дыра — это светлая область, НЕ связанная с краем кадра; заливаем
    от рамки и переворачиваем.
    """
    free = ~dark
    lab = label(free)
...
def measure(frame):
    ...
    # СИЛУЭТ МЕРЯЕТСЯ БЕЗ ВНУТРЕННИХ ДЫР. Ближняя рука в профиль нарисована в
    # НЕГАТИВЕ — светлой поверх чёрного плаща, иначе её не видно вовсе. Для
    # `widest_run`, который ищет самый длинный СПЛОШНОЙ тёмный отрезок, такая
    # рука разрезает плащ надвое: ширина профиля рухнула с 0.72 до 0.38, хотя
    # фигура не менялась ни на пиксель. Дыры (маска, негативная рука) сначала
    # заливаются: силуэт по определению сплошной, и мерить надо его.
    body = _open_x(_fill_holes(dark), 12)
    y45 = int(ys.min() + fig["h"] * 0.45)
    fig["w"] = widest_run(body, y45, y45 + 1)
