...
# Краски интонации — ТЕ ЖЕ КЛЮЧЕВЫЕ СЛОВА, что понимает завод
# (`voiceover.direct_line`). Список общий намеренно: гейт обязан ругаться на то
# же, что слышно в готовом mp3, иначе он проверяет собственную фантазию.
INTONATION = {
    "шёпот": ("шёпот", "шепот", "тихо", "тише"),
    "крик": ("крик", "кричи", "ярост", "зло", "злее", "рявк"),
    "нажим": ("жёстко", "жестко", "в упор", "оскал"),
    "рублено": ("рублен", "механич", "пулемёт", "пулемет"),
    "презрение": ("презрен", "ухмыл", "фамильярн", "дерзк"),
    "ирония": ("ирони", "насмешл", "язвит", "подкол", "с усмешкой"),
    "снисходительность": ("снисходит", "свысока", "покровительств", "с ленцой"),
    "тепло": ("тепл", "достоинств", "взлёт", "взлет"),
    "медленно": ("медленн", "расстановк", "финал", "весом", "жало"),
    "спокойно": ("спокойно", "диктор"),
}


def intonation(remark):
    """Краски интонации в ремарке. Пустое множество — ремарка вне словаря."""
    r = (remark or "").lower()
    return frozenset(name for name, keys in INTONATION.items()
                     if any(k in r for k in keys))


# Многосложные слова, которые ударения НЕ несут: предлоги-клитики. Ударение в
# «из-за пролитого чая» держит существительное, а размеченное «и́з-за»
# заставляет синтез бить по предлогу — так и звучало в «Гневе».
BEZUDARNYE = {"из-за", "из-под", "по-над", "из-подо"}
    ...
    for r in rows:
        for w in re.findall(rf"[А-Яа-яЁё{ACCENT}-]+", r["raw"]):
            ...
            if n_acc == 1:
                ...
                elif sum(c in VOWELS for c in base) < 2:
                    # ОДНОСЛОЖНОЕ С АКУТОЙ — БРАК, а не «знак нажима». В
                    # сценариях она стояла на «ва́с», «кто́», «мы́сль» с доводом,
                    # что синтез иначе роняет ударение на соседнее слово.
                    # Довод был догадкой; стенд `udarenie-proba2` его проверил:
                    # «ва́с» и «вас» дают РАЗНЫЕ записи (расхождение 0.19 при
                    # 0.15–0.35 у настоящей смены ударения). Менять акуте в
                    # односложном нечего — гласная одна, — значит она не
                    # подчёркивает слово, а портит его.
                    bad.append((r["n"], w, "акута на односложном: менять нечего, "
                                           "а слово портится"))
                continue
            if sum(c in VOWELS for c in base) < 2 or "ё" in base or "Ё" in base:
                continue
            if base.lower() in BEZUDARNYE:
                continue
...
def parse(path):
    ...
    for line in Path(path).read_text(encoding="utf-8").splitlines():
        ...
        if not re.match(r"VO-?\d", cells[0], re.I):
            continue
        # Реплика — ВСЕГДА ПОСЛЕДНЯЯ колонка, бит — предпоследняя. Считать от
        # начала (`cells[2]`, `cells[3]`) нельзя: сценарий с лишней служебной
        # колонкой съезжал, и гейт разбирал как речь название бита — ругался,
        # что в слове «ПРИВЕТСТВИЕ» не проставлено ударение, а настоящую
        # реплику не смотрел вовсе. `voiceover.py` это уже умеет и считает с
        # конца; здесь тот же разбор жил своей жизнью и фикса не получил.
...
def hits(rows, raw=""):
    ...
    # --- качель голоса (§2a) -------------------------------------------------
    # Одна эмоция, тянущаяся подряд, перестаёт читаться как эмоция и становится
    # фоном: смысл интонации возникает на СМЕНЕ. Машина проверяет только
    # различие соседних красок; что за иронией стоит именно снисходительность,
    # а не случайная другая краска, словарём не меряется и остаётся на человеке.
    tones = [(r["n"], intonation(r["remark"])) for r in rows]
    same = [f"{a} и {b} — {', '.join(sorted(t1))}"
            for (a, t1), (b, t2) in zip(tones, tones[1:])
            if t1 and t1 == t2]
    # Ремарка вне словаря завода звучит никак: её не переведут в темп/громкость.
    mute = [n for n, t in tones if not t]
    out.append((5, not same, "качель голоса: соседние реплики не в одной интонации",
                f"подряд одинаковая подача: {'; '.join(same[:3])}"))
    out.append((3, len(mute) <= len(rows) // 2,
                f"ремарки понятны заводу ({len(rows) - len(mute)}/{len(rows)})",
                f"ремарка вне словаря — звучит никак: {', '.join(mute[:6])}"
                + (" …" if len(mute) > 6 else "")))
