...
# --- словари правил ---------------------------------------------------------
ADDRESS = re.compile(r"\b(ты|тебя|тебе|тобой|вы|вас|вам|вами|твой|тво[ёяие]\w*|ваш\w*)\b", re.I)
...
def hits(rows):
    ...
    out.append((10, not WARMUP.search(first["text"]), "нет разогрева",
    ...
    out.append((7, t_first is not None and t_first <= 7.0, "хук укладывается в 7 секунд",
                f"первая реплика начинается на {t_first}с — зритель уже ушёл"
                if t_first else "не разобрал тайм-код"))
