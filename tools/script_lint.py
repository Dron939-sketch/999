...
# Индюшка, хорёк и индюшонок отсюда УЕХАЛИ В MISSTRESS: разметка их не
# переубедила, проверено на двух сборках подряд. Здесь остаётся то, где знак
# работает.
UNMARKED = [
    (r"насмерть", "на" + ACCENT + "смерть — ударение на А"),
]
