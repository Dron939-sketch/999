...
pub struct Surfaces {
    /// ЛОКАЦИЯ МОЖЕТ НЕ ИМЕТЬ ПОЛА ВОВСЕ, и это законное состояние, а не
    /// недоделанная карта. У «погасшего экрана» в кадре стол у правого края и
    /// глухая светлая стена — линии стыка не существует, ставить по ней
    /// нечего, и фигура размещается масштабом вручную, как в чёрных сценах.
    /// Раньше такой файл ронял загрузку («invalid type: null, expected struct
    /// Floor»), и локацию приходилось оставлять вовсе без карты — вместе с
    /// объяснением, почему пола нет. Теперь `"floor": null` читается как «пола
    /// нет», а `place ... on floor` в такой сцене ведёт себя ровно так же, как
    /// в локации без файла карты.
