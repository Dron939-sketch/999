...
## Персонаж из скульптуры (`freeman_rig`)

Форма и пропорции Фримена берутся из ОДНОЙ трёхмерной скульптуры
`examples/assets/characters/freeman_rig/source/mr_freeman.stl`, а не с отдельных
кадров. Так части не могут разойтись между собой: раньше плащ жил от одного
кадра, нога от другого, и ряд разворотов приходилось чинить гейтом, а не
рисунком.

Конвейер (порядок важен — второй шаг читает размеры, которые печатает первый):

```
python3 tools/stl_to_rig.py                    # STL → плоские SVG-части
python3 tools/apply_sculpt_proportions.py      # замер → геометрия скелета
python3 tools/karta.py --write                 # пересъём карты фигуры
python3 tools/turnaround.py --check            # гейт разворота
```

- **`tools/stl_to_rig.py`** — снимает подставку, привязывает треугольники к
  костям скульптуры (`tools/freeman_bones.json`), разворачивает каждую часть по
  её кости (из позы вынимается форма, а не поза) и пишет плоский двухтоновый
  силуэт в конвенции рига. Привязка проверяется ГЛАЗАМИ по `--segments`;
  правится `radius`/`priority`/`clip` в файле костей, а не код.
- **`tools/apply_sculpt_proportions.py`** — переводит замер в пивоты, масштабы и
  длины костей. Главная осторожность: в риге 124 позы, и они правят те же поля
  АБСОЛЮТНЫМИ значениями — каждой позе применяется та же правка, что базе,
  иначе правка молча пропадает в тех кадрах, где играет эта поза.
- **`tools/reframe_part.py`** — перекладывает готовый рисунок в другую рамку с
  другим пивотом. Нужен потому, что пивот принадлежит КОСТИ: у плаща три
  рисунка (анфас, три четверти, профиль), и они обязаны жить в одной рамке.

Что берётся со скульптуры и что нет — с обоснованием — в
`examples/assets/characters/freeman_rig/RULES.md`.

Правки в риге ломают golden-кадры эталона; после них:
