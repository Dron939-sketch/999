...
### Что пришло 08.09.2026 и что с этим было сделано

Четыре картинки сгенерированы и лежат в `main` под именами `NanoBanana_*.png`.
Все четыре попали в замысел; правки понадобились не по рисунку, а по кадру.

| промт | пришло | что сделано |
|---|---|---|
| §9 обложка | 1376×768, фигура вписана в рисунок | обрезка до 16:9 → `examples/assets/covers/filosofiya-intro-cover.png` |
| §1 автомат | 688×384 | обрезка и увеличение до 1280×720, обводка |
| §4 голова | **1024×1024**, квадрат | поле фона слева 600px до 16:9, достроена линия пола, обводка |
| §5 стол | 1376×768 | обрезка до 16:9, обводка |

**Голова пришла квадратом, и это была единственная настоящая работа.** Обрезка
до 16:9 срезала бы верх черепа, а фигуре стало бы негде встать — слева от
дверцы в квадрате всего 23% ширины. Вместо обрезки кадр расширен полем фона
слева. При этом обрывалась нарисованная от руки линия пола: она шла по всей
ширине квадрата и заканчивалась там, где начиналось поле. Линия достроена
тиражированием тонкой полосы с настоящей линией — так она осталась кривой от
руки, а не стала линейкой. Первая попытка тащила полосу шириной в 80 пикселей
и вместе с линией принесла обрывки штриха из-под неё; полоса сужена до
пятнадцати пикселей.

**Обводка, а не вставка растром.** `vectorize.py` определил все три как рисунок
(доля плоских пикселей 0.75–0.90 при пороге 0.45) и обвёл в настоящие пути.
Модуля `vtracer` в песочнице не было, и первый прогон молча свалился на
вставку растром — файлы вышли по 0.7–1.2 МБ и поплыли бы на сверхкрупе, где
камера наезжает до 2.01×. После установки vtracer: 290, 106 и 53 КБ путями.

**Ни одна из трёх не потребовала правки высоты.** Линия пола пришла на 0.82
(автомат), 0.76 (голова) и 0.60 (стол) — во всех трёх персонаж на 0.839 встаёт
ПЕРЕД ней, то есть ближе к камере, и это верно по глубине. Двигать картинку
под цифру 0.839 было бы ошибкой: 0.839 — это точка контакта ног, а не
обязательная высота горизонта.
