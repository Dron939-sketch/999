...
СТАТУС: ВСЕ ЧЕТЫРЕ КАРТИНКИ СГЕНЕРИРОВАНЫ И ПОДКЛЮЧЕНЫ. Промты оставлены,
чтобы можно было перегенерировать или доснять недостающее в том же стиле.

КАК ПОДКЛЮЧАЕТСЯ РАСТР. Движок грузит только SVG — `import set` разбирает файл
через usvg и на PNG падает с «provided data has not an UTF-8 encoding».
Поэтому растр заворачивается в SVG элементом `<image>` с data-URI:

    python3 tools/raster_set.py вход.png examples/assets/sets/имя.svg

Инструмент заодно снимает нарисованную рамку (в ролике она читается «картина
на стене»), кадрирует под 16:9 по центру со сдвигом вниз и квантует палитру до
двенадцати цветов — рисунок в две краски от этого не страдает, а файл худеет
втрое. Рядом кладётся исходный `.png`, чтобы картинку можно было переделать.

ЧЕГО ИНСТРУМЕНТ НЕ ДЕЛАЕТ: не пишет `*.surfaces.json`. Линию пола автомат
снять не может — у комнаты стена и пол одинаково светлые, и порог срабатывает
у самой нижней кромки. Карта ставится руками по рисунку, и без неё
`place ... on floor` молча не работает: фигура левитирует.
