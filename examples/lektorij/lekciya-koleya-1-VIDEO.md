...
## КАК Я БУДУ ПРОВЕРЯТЬ ПИЛОТ (список для лекции, а не для ролика)

1. **Длительности сходятся**: картинка, звук и выданный файл — с точностью до
   десятой. Ровно на этом ролик 25 собрался без финала.
2. **Звук не тронут.** Аудиозапись лекции идёт в файл БЕЗ мастеринга и без
   компрессии: она уже сведена, а `compose_video.sh` настроен на речь синтеза и
   поднимет то, что поднимать не надо. Проверяю побитово: дорожка в готовом
   файле должна совпасть с исходной.
3. **Смена кадра попадает в мысль**, а не в середину фразы: сверяю тайм-коды
   склеек с паузами в аудиодорожке — склейка обязана лечь в паузу.
4. **Кадр не стоит мёртвым.** На иллюстрации без персонажа глазу не за что
   ...
   движение камеры.
5. **Контактный лист по готовому файлу — глазами**, как и у роликов.
